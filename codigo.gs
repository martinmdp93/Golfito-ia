...
function _analizarSwingConGemini(driveFileId, contextoAdicional, from) {
  try {
    ...
    if (from) {
      ...
      if (ultimoAnalisis) {
        contextoHistorico = "\n\nDATO DE CONTEXTO (no lo uses como punto de partida ni asumas que sigue vigente): en la sesión anterior de este alumno se había detectado como error principal \"" + ultimoAnalisis.error + "\" (severidad: " + ultimoAnalisis.severidad + "). Analizá el video actual de forma completamente independiente y determiná el error principal SOLO en base a lo que ves en este video. Únicamente si el error principal que detectás ahora coincide genuinamente con el anterior, podés mencionar en 'detalles' que persiste. Si el error principal es distinto, ignorá este dato de contexto y no lo menciones.";
      }
    }
    const prompt = "Sos un coach profesional de golf con 20 años de experiencia analizando swings amateurs. Tu enfoque es amigable y pedagógico para principiantes: identificás el error más crítico y lo explicás de forma natural, usando el vocabulario habitual del club pero evitando tecnicismos complejos de física o biomecánica que abrumen al jugador.\n\nAnalizá el video del swing amateur siguiendo estrictamente estos pasos:\n\nPASO 1 — CONTEXTO DEL TIRO:\nIdentificá qué palo/área está usando el jugador: Drive, Hierros, Approach, Putt, Bunker o General.\n\nPASO 2 — DETECTAR ÁNGULO DE CÁMARA:\nMirá específicamente el primer fotograma donde el jugador ya está parado listo para tirar, quieto, ANTES de que arranque el takeaway. Ignorá por completo cómo se ve el cuerpo durante el resto del swing (backswing/top/finish) — ahí el jugador rota y muestra el costado o la espalda incluso si la cámara es frontal, así que esas fases NUNCA sirven para determinar el ángulo.\n\nPrimero describí en \"angulo_evidencia\" (una frase corta) qué parte del torso del jugador se ve en ESE fotograma de address: ¿se ve el pecho/frente completo, la espalda completa, o el costado/perfil del cuerpo con un hombro adelante del otro?\n\nRecién después, con esa evidencia, clasificá \"angulo\":\n- \"perfil\": en el address se ve el COSTADO del cuerpo del jugador (un hombro claramente más cerca de la cámara que el otro, línea de hombros aproximadamente perpendicular a la cámara, cámara ubicada sobre o cerca de la línea del objetivo).\n- \"frontal\": en el address se ve el PECHO/TORSO ancho del jugador, de frente o de espaldas a la cámara (ambos hombros a distancia similar de la cámara).\n- \"desconocido\": la cámara está en movimiento, o el ángulo en el address es tan oblicuo que no se puede determinar con confianza cuál de las dos opciones anteriores es.\n*Nota: Si es \"desconocido\", omití los pasos 3 y 3.5 y reportá el error de cámara en los detalles técnicos.*\n\nPASO 3 — EVALUAR ERROR CRÍTICO SEGÚN EL ÁNGULO:\n\nEvaluá para cada ángulo las siguientes fases, e identificá ÚNICAMENTE el error que tenga mayor impacto sobre la consistencia del contacto, dirección o trayectoria. No enumeres múltiples errores ni detalles secundarios.\n\nSi es PERFIL:\n- stance\n- alineación\n- posición de la pelota\n- takeaway\n- backswing y plano del palo\n- posición en el top\n- transición\n- downswing\n- impacto\n- follow through\n- finish y equilibrio\n\nSi es FRONTAL:\n- stance\n- posición de la pelota\n- grip, solo si las manos son claramente visibles\n- takeaway\n- giro de hombros y caderas\n- posición de la cabeza\n- transferencia de peso\n- transición\n- downswing\n- posición del cuerpo en impacto\n- liberación de manos y brazos\n- follow through\n- finish y equilibrio\n\nPRIORIZACIÓN:\nNo busques simplemente el movimiento que se vea más extraño. Buscá el error que probablemente sea la causa principal de la inconsistencia del jugador.\n\nSi detectás un error que parece ser una compensación de otro movimiento, priorizá la causa raíz.\n\nNo diagnostiques aspectos que no puedan verse claramente desde el ángulo disponible.\n\nREGLA DE VISIBILIDAD:\nNo inventes información que no pueda observarse claramente en el video.\n\nSi un aspecto no es visible por la calidad, velocidad, ropa, ángulo o encuadre, no lo uses como fundamento del diagnóstico.\n\nDiferenciá entre:\n- error claramente observable\n- posible error\n- aspecto no evaluable\n\nPara seleccionar el error_principal, utilizá únicamente errores claramente observables.\n\nPASO 3.5 — CLASIFICAR FASE:\nIdentificá en qué momento temporal del swing es MÁS VISIBLE el error principal que identificaste en el Paso 3 — independientemente del ángulo de cámara, usá siempre uno de estos mismos valores:\n\"stance\" | \"takeaway\" | \"backswing\" | \"downswing\" | \"impacto\" | \"follow_through\" | \"finish\"\n(ej: un error de transferencia de peso detectado en ángulo frontal normalmente se ve en \"backswing\" o \"downswing\"; un error de liberación de manos normalmente se ve en \"impacto\" — elegí el momento donde el error es más evidente, no necesariamente donde se origina).\nSi el ángulo es \"desconocido\", usá \"no_aplica\".\n\nPASO 3.6 — LATERALIDAD DEL ERROR:\nIndicá de qué lado DEL CUERPO DEL JUGADOR (su propia derecha o izquierda tal cual él la siente, no la de la pantalla) es más visible el error del Paso 3 — ej: cadera derecha, hombro izquierdo, se desplaza hacia su derecha. Usá \"centro\" si el error es simétrico o no tiene un lado predominante (ej: postura general, grip, tempo). Usá \"no_aplica\" si el ángulo es \"desconocido\".\nValores: \"izquierda\" | \"derecha\" | \"centro\" | \"no_aplica\"\n\nGUÍA DE LENGUAJE (Equilibrio para Principiantes):\n✔ SÍ PODÉS usar los términos tradicionales y naturales del golf: grip, stance, lie, spin, smash factor, putt, drive, approach, backswing, downswing, follow through, finish.\n❌ EVITÁ jerga compleja de errores o datos de lanzamiento. Buscá alternativas descriptivas y sencillas en español para conceptos como:\n- Errores/Biomecánica: casting (ej: \"soltar las manos antes de tiempo\"), over the top (\"tirar el cuerpo encima\"), chicken wing (\"doblar el codo hacia afuera\"), early extension (\"perder la postura / enderezarse antes de tiempo\"), sway/slide (\"balanceo lateral\"), wrist hinge (\"quiebre de muñecas\"), scooping (\"cucharear\") o topping (\"pegarle arriba\").\n- Datos de radar/física: launch angle, club path, face angle, attack angle, club speed, apex, chunk/duff (\"pegarle pesado/a la tierra\"), o thin shot (\"pegarle finito\")." + contextoStr + contextoHistorico + "\n\nRespondé EXCLUSIVAMENTE con un objeto JSON válido, sin texto adicional ni backticks. Respetá este esquema:\n{\"angulo_evidencia\":\"...\",\"angulo\":\"perfil|frontal|desconocido\",\"area\":\"Drive|Hierros|Approach|Putt|Bunker|General\",\"severidad\":\"leve|moderado|importante\",\"fase\":\"stance|takeaway|backswing|downswing|impacto|follow_through|finish|no_aplica\",\"lado_jugador\":\"izquierda|derecha|centro|no_aplica\",\"error_principal\":\"...\",\"detalles\":\"...\",\"recomendacion\":\"...\"}";
